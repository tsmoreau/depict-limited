...
// File: contracts\PhasesProject.sol

contract PhasesProject is Ownable, ERC721Token, ERC721Holder {
    using Strings for string;
    using Integers for uint;

    function PhasesProject () ERC721Token("Phases/Faces" ,"PHASES") public {
    ...
    mapping(address => mapping (uint256 => uint256)) internal mintedTokensIndex;

    uint128 public mintPhaseCost = 0.0 ether;
    string preUri1 = "000";
    string preUri2 = "111";
    ...
    function setMintPhaseCost(uint128 _fee) public onlyOwner {
        mintPhaseCost = _fee;
    }
    ...
    function setPreUri2(string _preUri) public onlyOwner {
     ...
     function getNextPhase() public view returns (string memory) {
        return PhaseData[0];
    }
    ...
    function mintedCountOf(address _minter) public view returns (uint256) {
        return mintedTokensCount[_minter];
    }

    function mintedTokenOfOwnerByIndex(address _minter, uint256 _index) public view returns (uint256) {
        require(_index < mintedCountOf(_minter));
        return mintedTokens[_minter][_index];
    }
    ...
    function mintPhase() public payable {
        require(msg.value == mintPhaseCost);
        Phase memory _Phase = Phase({ipfsHash: PhaseData[ArrayPosition+1], minter: msg.sender});
        uint256 newPhaseId = phaseInfo.push(_Phase) - 1;
        ipfsHashToTokenId[PhaseData[ArrayPosition+1]] = newPhaseId;
       
        _mint(msg.sender, newPhaseId);
        mintedTokensCount[msg.sender]++;
        uint256 length = mintedTokens[msg.sender].length;
        mintedTokens[msg.sender].push(newPhaseId);
        mintedTokensIndex[msg.sender][newPhaseId] = length;
    ...
    function tokenURI(uint256 _tokenId) public view returns (string memory) {
        require(exists(_tokenId));
        return phaseInfo[_tokenId].ipfsHash;
    }
